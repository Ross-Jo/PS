...
// D[a][b][c][p1(전날)][p2(전전날)] = A, B, C의 갯수가 (a,b,c)이고, 전날 일한 사람이 p1, 그 전날 일한 사람이 p2인 것이 가능한가? 

public class Q14238_2 {
	...
	static int go(int a, int b, int c, int p1, int p2) {
		...
		if (b+1 <= limit[1] && p1 != 1 && go(a, b+1, c, 1, p1) == 1) {
			d[a][b][c][p1][p2] = 1;
			return d[a][b][c][p1][p2];
		}
		
		if (c+1 <= limit[2] && p1 != 2 && p2 != 2 && go(a, b, c+1, 2, p1) == 1) {
			d[a][b][c][p1][p2] = 1;
			return d[a][b][c][p1][p2];
		}
		d[a][b][c][p1][p2] = 0;
	...
	public static void main(String args[]) {
		...
		n = s.length();
		for (int i=0; i<n; i++) limit[s.charAt(i)-'A'] += 1;
